...
public class ImmutablesSupplier extends TestDataSupplier<Response> {
	...
	public Response make(int times) {
		ImmutableResponse.Builder response = ImmutableResponse.builder();
		response.status("success");
		response.isRealJson(true);
		for (int i = 0; i < times; i++) {
			ImmutableUser.Builder u = ImmutableUser.builder();
			...
			u.name(ImmutableName.builder().first("Lora").last("Peterson").build());
			...
			response.addUsers(u.build());
		}
		return response.build();
	}

	Friend friend(int id, String name) {
		return ImmutableFriend.builder().id(id).name(name).build();
	}
	...
	Image image(String id, String format, String url, String description) {
		return ImmutableImage.builder().id(id).format(format).url(url).description(description).build();
	}
}
